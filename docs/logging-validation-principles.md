...
Les logs nginx passent par le collecteur OTel qui les scrubbe (emails, IP, query du referrer). **Les logs applicatifs vont directement à PostHog et contournent ce pipeline** : la discipline est donc à l'appelant — ne pas mettre de PII dans une ligne — et la protection PII de PostHog est activée côté projet comme filet. Le logger ne masque rien : pino n'offre aucun mécanisme à toute profondeur (pas de regex, `*` vaut un seul segment, `**` n'existe pas, et la doc de son moteur annonce pour `*` un « any level » que le code ne fait pas), donc une liste de chemins donnerait une fausse assurance — constaté : `*.email` ne couvre pas `ngc.user.profile.email`. Côté export, la censure par nom de segment qui avait été essayée a été retirée avec le reste : masquer à l'export laisserait passer la même donnée sur stdout. `userId` et `sessionId` PostHog sont les seules données identifiantes admises — exposition déjà couverte par le consentement analytics/replay. Les payloads métier (`situation`, `computedResults`, `foldedSteps`) n'ont rien à faire dans une ligne non plus, mais par volume : ce ne sont pas des PII, c'est à l'appelant de ne pas les passer.
...
- **`scope` nomme l'unité qui émet**, sous la forme `paquet.couche.unité` — le mot de la spec pour ça (`InstrumentationScope`) :
  ...
  **service** sert ces gestes — y compris quand il est exposé au client depuis
  ...
  `worker` la boucle du worker par itération de job, `middleware`,
  `instrumentation` et `sideEffect` complètent la liste. La forme
...
- **Les attributs sont plats, et c'est la fabrique qui les aplatit** : un objet de `meta` — ou d'un binding de `child` — devient des clés pointées (`engine.key`), séparateur que les noms semconv emploient eux-mêmes (`http.request.method`), et la même forme part sur stdout et vers PostHog. L'aplatissement est une contrainte du backend — PostHog ne le fait pas (vérifié par sonde : un `kvlistValue` y arrive en JSON texte et sa colonne pointée est nulle) — mais il est fait une fois, en amont, pour que les deux sorties ne divergent jamais ; si un `flatten` d'ingestion prenait le relais un jour (comme le collecteur des logs nginx), c'est cette étape qu'on supprimerait. Au-delà de quatre niveaux, la valeur reste du JSON, faute de mieux ; la troncature, elle, reste à l'export — c'est une limite de PostHog, la ligne garde sa valeur entière. L'exception d'une ligne prend `exception.type` et `exception.message` — pas la stack, qui est celle de Sentry ; une `Error` glissée dans la `meta` n'est pas un cas traité (une seconde erreur se lie par `cause`).
